...
export default function AppStoreButton({ className = "", size = "md" }: Props) {
  ...
  const inner = (
    <span
      className={`group flex w-full min-h-[44px] items-center justify-center gap-3 rounded-2xl border-2 border-ink/15 bg-white/70 backdrop-blur sm:w-auto sm:justify-start ${dims} text-ink shadow-soft transition-[transform,border-color,box-shadow] duration-200 hover:-translate-y-0.5 hover:border-ink/30 hover:shadow-card active:translate-y-0`}
    >
      <svg
        viewBox="0 0 24 24"
        className={`shrink-0 text-ink ${size === "lg" ? "h-8 w-8" : "h-7 w-7"}`}
        fill="currentColor"
        aria-hidden
      >
